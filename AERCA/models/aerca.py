...
class AERCA(nn.Module):
    ...
    def encoding(self, xs):
        ...
        preds, coeffs = self.encoder(winds, mask=self.causal_mask) 
        us = preds - nexts
    ...
    def decoding(self, us, winds, add_u=True):
        ...
        preds, coeffs = self.decoder(u_winds, mask=self.causal_mask)
        prev_preds, prev_coeffs = self.decoder_prev(winds, mask=self.causal_mask)
